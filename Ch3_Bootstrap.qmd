...
#| fig.align: center
#| dev: png
#| fig-cap: "Animated illustration of the Glivenko-Cantelli theorem: the empirical CDF converges uniformly to the true CDF."

##############################################################################
# Animated Glivenko-Cantelli plot
##############################################################################
...
x.grid <- seq(-4, 4, length.out = 1000)
F.true <- pnorm(x.grid)

for(n in seq(5, n.max, by = 5)){
  
  F.emp <- ecdf(x[1:n])(x.grid)
  sup.diff <- max(abs(F.emp - F.true))
  ...
  legend("bottomright",
         legend = c("True CDF F(x)", "Empirical CDF F_n(x)",
                    expression("|F"[n]*"(x) - F(x)|")),
...
* Moreover, for simplicity let us focus on **unbiased** and $\boldsymbol{\sqrt{n}}$**-consistent** estimators, i.e.
  ...
  * $\operatorname{SE}\left(\hat\theta_n\right)=\sqrt{Var\left(\hat\theta_n\right)}\leq \frac{1}{\sqrt{n}}\cdot\text{constant}$
...
However, deriving a useful, *explicit* expression of the asymptotic variance $\lim_{n\to\infty}n\cdot Var\big(\hat\theta_n\big)$ can be *very* hard (see @sec-Illustration). Then, one is happy to use the bootstrap instead of doing painful math. Moreover, specific versions of the Bootstrap (@sec-BootT) can be even more accurate than an asymptotic normality result. 
...
$$
3. **Bootstrap replications:** Repeat Steps 1 and 2 $m$ times (for a large value of $m,$ such as $m=10000$) leading to $m$ bootstrap estimates 
$$
...
1. The bootstrap sampling process must reflect the original sampling process. For instance:  
...
> Theorem 1 in @Mammen_1992_Book shows that the basic bootstrap is consistent if $\sqrt{n}(\hat{\theta}_n-\theta_0)\to_d\mathcal{N}(0,v_0^2),$ under the assumption that the bootstrap sampling process (e.g. i.i.d.) equals the original sampling process.  
...
::: {.callout-note appearance="minimal"}
::: {#exm-AsympNorm}
# Recap: Inference about $\mu_0$ using Classic Asymptotic Statistics 
By the Lindeberg-Lévy CLT we know that 
$$
...
$$
where $H^{Asymp}_n(x)\to\Phi_{\sigma_0}(x)$ as $n\to\infty$ for all continuity points $x.$
That is, we use the approximate distribution
$$
\begin{align*}
...
$${#eq-SEAsymp}
Using $\widehat{\operatorname{SE}}^{Asymp}_{n}\big(\bar{X}_n\big),$ we can compute confidence intervals and statistical tests in the usual way. 

**Note:** The estimator of the standard error in @eq-SEAsymp requires a formula for the asymptotic variance $v^2=\sigma_0^2$ as derived in @eq-AsympVarDeriv. Such derivations can be very challenging (see @sec-Illustration).
:::
:::
...
Before we answer this question theoretically (see @sec-Theory1 and @sec-Theory2BootstrapConsist), we check it empirically using an **artifical data example.** This is then, of course, not a generally valid mathematical proof.

#### **Practice: Empirical Consideration of the Bootstrap distribution $\boldsymbol{H^{Boot}_{n}}$** {#sec-PracticeXbar}
...
This setup implies (see @exm-AsympNorm) that
...
When doing classic asymptotic inference (see @exm-AsympNorm), we would use the feasible 
$$
...
$$
by (i.i.d.) sampling $n=100$ data points from a $\chi^2$-distribution with $\operatorname{df}=2.$ The first six data points are shown in @tbl-ChiSqSample. 
...
: First six data points of the observed realization of the random sample $\mathcal{S}_n=\{X_1,\dots,X_n\}$ with sample size $n=100$ drawn from a $\chi^2$-distribution with $\operatorname{df}=2.${#tbl-ChiSqSample}

So the observed sample mean is 
<center>
$\bar X_{n} =$ `mean(observedSample)` $=$ `r round(mean(observedSample),2)`
</center>
...
* $H^{Boot}_{n,m}(x) =$ `ecdf( sqrt(n) * (Xbar_boot - Xbar) )` </br>
...
#| eval: true
#| label: fig-Compare
#| fig-cap: Both $H^{Boot}_{n,m}(x)$ and $H^{Asymp}_{n}(x)$ provide here good approximations to the (usually unknown) asymptotic limit distribution $\Phi_{2}(x).$
...
\end{align*}
$$
can be computed **directly** from $\bar{X}^*_{n,1},\dots,\bar{X}^*_{n,m}.$ There's no need for deriving a formula $v^2=\sigma_0^2$ for the asymptotic variance as it is necessary for $H^{Asymp}_{n}(x)$ (see @exm-AsympNorm).   
...
$$ 
can be computed *directly* from $\bar{X}^*_{n,1},\dots,\bar{X}^*_{n,m}.$ There's no need for deriving a formula $v^2=\sigma_0^2$ for the asymptotic variance as in @exm-AsympNorm. For our artifical data example, we have that 
<center>
$\hat{v}^2_{Boot,n}=$ `n * var(Xbar_boot_vec)` $=$ `r round(n * var(Xbar_boot_vec), 2)`
</center>
</br>
...
<center>
$\widehat{SE}_{Boot,n}\left(\bar{X}_n\right)=\sqrt{\frac{\hat{v}^2_{Boot,n}}{n}}\;=$ `sd(Xbar_boot_vec)` $=$ `r round(sd(Xbar_boot_vec), 2)`
</center>
...
#### Excursus: Complex Numbers and the characteristic function 

In probability and econometrics, we often want to describe the distribution of a random variable.

The most familiar object is the cumulative distribution function:

$$
F_X(x) = P(X \leq x).
$$

However, when we study sums of independent random variables, such as

$$
S_n = X_1 + X_2 + \cdots + X_n,
$$

the CDF of the sum is often difficult to handle directly.

Characteristic functions provide another way to represent a distribution. They are less intuitive at first, but they are very useful because they turn sums of independent random variables into products.

To understand characteristic functions, we need to understand complex numbers.
...
A useful way to think about $z=a+bi$ is as a point in the plane:

$$
a+bi \quad \leftrightarrow \quad (a,b).
$$

So complex numbers are essentially two-dimensional numbers with special rules for multiplication.

```{r}
#| label: fig-complex-plane
#| fig-cap: "The complex number z = 2 + i as a point in the complex plane."
...
points(2, 1, pch = 19)
arrows(0, 0, 2, 1, length = 0.1)
text(2, 1.15, labels = "z = 2 + i")
```
...
The central formula we need is **Euler's formula:**

$$
e^{i\theta} := \cos(\theta) + i\sin(\theta).
$$

This tells us that $e^{i\theta}$ is a point on the unit circle.

Its real part is $\cos(\theta)$, and its imaginary part is $\sin(\theta)$.
...
So $e^{i\theta}$ always lies on the unit circle.

```{r}
#| label: fig-unit-circle
#| fig-cap: "Euler's formula places e^{i theta} on the unit circle."
...
points(cos(theta0), sin(theta0), pch = 19)
...
Now let $X$ be a random variable.

For any real number $t$, consider
...
So $e^{itX}$ is a complex-valued random variable.

It always lies on the unit circle because

$$
|e^{itX}|=1.
$$

This fact is important. Since $|e^{itX}|=1$, the expectation of $e^{itX}$ always exists.

We are now in the position to define the concept of a characteristic function.
...
The name "characteristic function" is not accidental.

Two random variables $X$ and $Y$ have the same characteristic function, 
...
Characteristic functions behave very nicely under sums.
...
> The characteristic function turns sums of independent random variables into products of functions.

**Example**: Characteristic function of a normal random variable
...
$$
g(t)=\varphi_Z(t)=\mathbb{E}[e^{itZ}].
$$

Using the standard normal density,
...
$$
g'(t)
= \underset{=0}{[-ie^{itz}f]_{-\infty}^{\infty}}-(-i)\int_{-\infty}^{\infty}
it e^{itz}f(z)\,dz =
-tg(t).
$$
...
=
e^{i\mu t}
\mathbb{E}[e^{i(\sigma t)Z}].
$$

Since

$$
\mathbb{E}[e^{iuZ}]=e^{-u^2/2},
$$

with \(u=\sigma t\), we obtain
...
Larger variance means faster damping.

```{r}
#| label: fig-normal-cf
#| fig-cap: "Real and imaginary parts of the characteristic function of N(1, 1)."

mu <- 1
sigma <- 1

t <- seq(-6, 6, length.out = 500)

damping <- exp(-0.5 * sigma^2 * t^2)
real_part <- damping * cos(mu * t)
imag_part <- damping * sin(mu * t)
...
plot(
  ...
  xlab = "t",
  ylab = "Value",
  main = "Characteristic function of N(1, 1)"
)

lines(t, imag_part, lty = 2)
abline(h = 0, lty = 3)

legend(
  "bottomleft",
  legend = c("Real part", "Imaginary part"),
  lty = c(1, 2),
  bty = "n"
)
```
...
Lastly, we need to be able to approximate characteristic functions locally.
...
> The following is taken from Example 3.1 in @Shao_Tu_1996. First, we sketch the proof for the Lindeberg-Lévy CLT. Second, we use this principle to sketch the proof for bootstrap consistency. 
...
&\leq t^2\,\mathbb{E}\left(|t|\,n^{-3/2}\left|X-\mu_0\right|^3\right)\\[2ex]
...
\end{align*}
$$
where the latter step follows from @eq-CharacNormal.
...
\end{align*}
$$
where, assuming $\mathbb E[|W|^3]<\infty$ for convenience,
$$
\begin{align*}
|\lambda_n^*(t)|
...
& \leq |t^3|\,{\color{blue}\frac{1}{n^{\frac 52}}\sum_{i=1}^n}\left|X_i-\bar{X}_n\right|^3
\end{align*}
$$
In particular, 
$$
...
\Leftrightarrow\quad {\color{red}|\lambda^*_n(t)|}&=o_{a.s.}\left(\frac 1n \right).
\end{align*}
$$
...
**Note:** Often, however, very difficult to obtain a consistent estimator $\hat v_n$ of $v_0$ (see @sec-Illustration). Statistical inference is then usually based on the **bootstrap confidence intervals**. 
::: 
...
**Good to know:** Theorem 1 in @Mammen_1992_Book shows that the basic bootstrap is consistent if $\sqrt{n}(\hat{\theta}_n-\theta_0)\to_d\mathcal{N}(0,v_0^2),$ under the assumption that the bootstrap sampling process (e.g. i.i.d.) equals the original sampling process.
:::
:::
...
3. The $(1-\alpha)\times 100\%$  **basic bootstrap confidence interval** is then given by 
$$
...
2. Compute the empirical quantiles $\hat q^*_{n,\frac{\alpha}{2}}$ and $\hat q^*_{n,1-\frac{\alpha}{2}}$ from $\bar X^*_{n,1},\bar X^*_{n,2},\dots,\bar X^*_{n,m}$ using @eq-empiricalQuantile. 
...
## The Bootstrap-$\boldsymbol{t}$ Method {#sec-BootT}
...
$$
In this case $T_n$ is an **asymptotically pivotal statistic**.</br></br> 
...
### The Bootstrap-$\boldsymbol{t}$ Confidence Interval 
...
**Justifying the Bootstrap-$\boldsymbol{t}$ CI (@eq-Boot_tCI) for $\boldsymbol{\theta_0}$:**
...
### Accuracy of the Bootstrap-$\boldsymbol{t}$ method 
...
* @sec-bootPairs considers the **Bootstrapping Pairs** algorithm  
* @sec-bootResid considers the **Residual Bootstrap** algorithm. 
...
X_i:=(\underbrace{X_{i1}}_{=1},X_{i2},\ldots,X_{ip})^T\in\mathbb{R}^p,
$$
under the **fixed design** (@def-RandomFixedDesign), where 
$$
...
$$
where $\hat\beta_n$ is computed from the original sample.
Repeatedly ($m$ times) generating such bootstrap samples allows generating $m$ realizations of the bootstrap estimator
$$
...
1. The wild bootstrap (@Mammen_1993) uses 
$$
...
\end{align*}
$$
See @Mammen_1993 for a detailed discussion of the properties of this method.
2. The second method is an example of the **multiplier bootstrap**, which uses 
$$
...
where 
   ...
   * $U_1,\dots,U_n\overset{\text{i.i.d.}}{\sim}U$ are real valued i.i.d. random variables, independent of the residuals $\hat{\varepsilon}_i,$ and with $\mathbb{E}(U)=0$ and $\mathbb{E}(U^2)=1.$ One possibility would be $U\sim\mathcal{N}(0,1).$ Another often used possibility is to use a [Rademacher distributed](https://en.wikipedia.org/wiki/Rademacher_distribution) random variable $U.$
   
See @Davidson_and_Flachaire_2008 for a detailed discuss of the properties of this method. 





### Bootstrap Confidence Intervals for the $\boldsymbol{j}$th Component of the Regression Coefficient $\boldsymbol{\beta_{0}}$
...
#### **Basic Bootstrap Confidence Intervals for $\boldsymbol{\beta_{0,j}}$** 
...
#### **Bootstrap-$\boldsymbol{t}$ Confidence Intervals for $\boldsymbol{\beta_{0,j}}$**
...
1. Use either **bootstrapping pairs** (@sec-bootPairs) for random designs, or **bootstrapping residuals** (@sec-bootResid) for fixed designs and homoskedastic errors, or the **wild bootstrap** (@sec-bootWild) for fixed desgins and heteroskedastic errors, to generate $m$ (e.g. $m=10,000$) bootstrap realizations
$$
...
where 
   ...
   * $\widehat{\operatorname{SE}}(\hat\beta^*_{j,n})$ is an appropriate estimate of the standard error (e.g. @eq-homoskedSE or @eq-heterokedSE)
...
* $\beta_{0,j}$ denotes the $j$th element of $\beta_0\in\mathbb{R}^p,$ 
* $\hat\beta_{j,n}$ denotes the $j$th element of the OLS estimator $\hat\beta_n\in\mathbb{R}^p.$ 
* $\widehat{\operatorname{SE}}(\hat\beta_{j,n})$ is an appropriate estimator of the standard error (e.g. @eq-homoskedSE or @eq-heterokedSE)
...
$g_n(\cdot)$ converges **uniformly** to $g(\cdot)$ if for every $\varepsilon>0,$ there exists an $N_\varepsilon$ such that 
\begin{align*}
...
\end{align*} 
Thus, we can proceed by computing the mean $\mathbb{E}\left(\left.Y^*\right|\mathcal{S}_n\right)$ of a discrete random variable $\left(\left.Y^*\right|\mathcal{S}_n\right)$ such that 
\begin{align*}
\mathbb{E}^*(\bar{Y}^*) 
